...
Fork [OpenLid](https://github.com/ameeetgaikwad/OpenLid), clone your fork, and make a focused branch for your change. Read the [architecture](docs/architecture.md) for the capture and render flow and [roadmap](ROADMAP.md) for priorities.
...
4. For rendering, sensor, or lifecycle changes, run [manual validation](docs/manual-validation.md) and report exactly which checks you ran.
...
Follow the existing Swift style and macOS 14 API baseline. Keep timer and overlay lifecycle work on the main actor, and document any new OS or hardware requirement. Do not reset other contributors' edits or include local build/session files in your PR.

## Project expectations

Keep the app local-only. Do not introduce frame storage, telemetry, network dependencies, privileged helpers, or permission escalation without a clearly discussed need. Invalid or stale sensor input must restore the real desktop. The overlay must stay click-through and hide on pause. Capture must exclude only the effect overlay and stop cleanly.
