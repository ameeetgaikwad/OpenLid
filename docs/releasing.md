...
The static `website/` directory is hosted on Vercel at https://open-lid.vercel.app/. Configure the Vercel project with root directory `website`, framework preset Other. The committed `website/vercel.json` sets `npm run build` and output directory `dist`. Run `npm ci` and `npm run build` in `website/` to build locally. No environment variables are needed. Enable Web Analytics in the Vercel project dashboard to collect website page views. GitHub Pages is not used. Download buttons point to assets hosted separately on GitHub Releases. Its interactive preview is illustrative and does not read the visitor's lid sensor or capture their screen.
